...
add_subplot(a,b,c)          - a=NumOfRow, b=NumOfCol, c=PlotNum
...
no_diseaseN = df[(df['target'] == 0)]   # for "no disease" plot
diseaseN    = df[(df['target'] == 1)]   # for "disease" plot
df2 = df[['sex','cp','slope','ca']]   # picking a few rows

#  fig.add_subplot([# of rows] by [# of columns] by [plot#])
plotRow = len(df2.columns)
plotCol = 3  # three columns: overall, no disease, disease
...
for i in df2.columns:
    # OVERALL plots
    fig.add_subplot(plotRow, plotCol, plotNum)
    plt.title('OVERALL (row{},col{},#{})'.format(plotRow, plotCol, plotNum), fontsize=14)
    ...
    # NO DISEASE PLOTS
    fig.add_subplot(plotRow, plotCol, plotNum)
    plt.title('NO DISEASE (row{},col{},#{})'.format(plotRow, plotCol, plotNum), fontsize=14)
    plt.xlabel(i, fontsize=12)
    sns.countplot(no_diseaseN[i], hue=df.sex)
    plotNum = plotNum + 1

    # PLOTS
    fig.add_subplot(plotRow, plotCol, plotNum)
    plt.title('DISEASE (row{},col{},#{})'.format(plotRow, plotCol, plotNum), fontsize=14)
    plt.xlabel(i, fontsize=12)
    sns.countplot(diseaseN[i], hue=df.sex)
    plotNum = plotNum + 1
